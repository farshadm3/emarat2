...
@section('js')
    <script>
        ...
        function createThird(PARENT, TABLE_ID = 'third_banner_table') {
            // $('.page-loader-wrapper').show();
            $('#btnThird').text('Save');
            $('#createThirdTitle').html('Create Third');
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('third_banners.store') }}",
                data: JSON.stringify(data),
                success: function ({third_banner, msg}) {
                    $('#' + TABLE_ID).append(
                        `
                            <tr class="py-5 fw-bold  border-bottom border-gray-300 fs-6">
                                <td class="text-center">${third_banner.id}</td>
                                <td class="text-center">${third_banner.link}</td>
                                <td class="text-center">
                                    <div class="image-input image-input-circle" data-kt-image-input="true">
                                        <div class="image-input-wrapper w-50px h-50px">
                                            <img src="${third_banner.image}"/>
                                        </div>
                                    </div>
                                </td>
                                <td class="text-center">${third_banner.created_at}</td>
                                <td class="text-center">
        ...
        function deleteModalVal() {
            ...
            $('#image').val("");
            $('#btnThird').text('save');
            $('#createThirdTitle').html('create third_banner');
        }
        ...
        function setAttrToCatBtn(CAT_ID, TABLE_ID = "mainTable") {
            $('#btnThird').attr('onclick', `createThird("${CAT_ID}", "${TABLE_ID}")`)
        }
                    ...
                    <div class="col-xxl-4">
                        ...
                        <div class="card card-xxl-stretch">
                            ...
                            <div class="card-header align-items-center border-0 mt-3">
                                ...
                                <div class="col-lg-12">
                                    <table class="table table-rounded table-flush" id="third_banner_table">
                                        <thead>
                                        ...
                                        <tbody>
                                        @foreach($third_banners as $third_banner)
                                            <tr class="py-5 fw-bold  border-bottom border-gray-300 fs-6">
                                                <td class="text-center">{{$third_banner->id}}</td>
                                                <td class="text-center">{{$third_banner->link}}</td>
                                                <td class="text-center">
                                                    <div>
                                                        <div>
                                                            <img  class="image-input image-input-circle image-input-wrapper w-50px h-50px" src="{{$third_banner->image}}"/>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="text-center">{{$third_banner->created_at}}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="createThirdTitle">Create Banner</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="btnThird" onclick="createThird(0)">Save</button>
                </div>
            </div>
        </div>
    </div>
