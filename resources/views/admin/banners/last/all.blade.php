...
@section('js')
    <script>
        ...
        function createLast(PARENT, TABLE_ID = 'last_banner_table') {
            // $('.page-loader-wrapper').show();
            $('#btnLast').text('Save');
            $('#createLastTitle').html('Create Last');
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('last_banners.store') }}",
                data: JSON.stringify(data),
                success: function ({last_banner, msg}) {
                    $('#' + TABLE_ID).append(
                        `
                            <tr class="py-5 fw-bold  border-bottom border-gray-300 fs-6">
                                <td class="text-center">${last_banner.id}</td>
                                <td class="text-center">${last_banner.link}</td>
                                <td class="text-center">
                                    <div class="image-input image-input-circle" data-kt-image-input="true">
                                        <div class="image-input-wrapper w-50px h-50px">
                                            <img src="${last_banner.image}"/>
                                        </div>
                                    </div>
                                </td>

                                <td class="text-center">${last_banner.created_at}</td>
                                <td class="text-center">
        ...
        function deleteModalVal() {
            ...
            $('#image').val("");
            $('#btnLast').text('save');
            $('#createLastTitle').html('create last_banner');
        }
        ...
        function setAttrToCatBtn(CAT_ID, TABLE_ID = "mainTable") {
            $('#btnLast').attr('onclick', `createLast("${CAT_ID}", "${TABLE_ID}")`)
        }
                    ...
                    <div class="col-xxl-4">
                        ...
                        <div class="card card-xxl-stretch">
                            ...
                            <div class="card-header align-items-center border-0 mt-3">
                                ...
                                <div class="col-lg-12">
                                    <table class="table table-rounded table-flush" id="last_banner_table">
                                        <thead>
                                        ...
                                        <tbody>
                                        @foreach($last_banners as $last_banner)
                                            <tr class="py-5 fw-bold  border-bottom border-gray-300 fs-6">
                                                <td class="text-center">{{$last_banner->id}}</td>
                                                <td class="text-center">{{$last_banner->link}}</td>
                                                <td class="text-center">
                                                    <div>
                                                        <div>
                                                            <img  class="image-input image-input-circle image-input-wrapper w-50px h-50px" src="{{$last_banner->image}}"/>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="text-center">{{$last_banner->created_at}}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="createLastTitle">Create Banner</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="btnLast" onclick="createLast(0)">Save</button>
                </div>
            </div>
        </div>
    </div>
